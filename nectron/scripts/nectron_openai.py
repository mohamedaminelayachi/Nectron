...
class NectronOpenAI:
    ...
    def __init__(self, model_id: str, ablation: bool = False):
        self.__client = None
        self.__model = None
        if not ablation:
            ...
            with open('./prompts/prompts_ablation.json', 'r', encoding='utf8') as fp:
                self.__prompts = json.load(fp)
        self.__pd = None
    ...
    def InitClient(self, api_key: str = None, api_key_path: str = None, num_requests: int = 20, sleep: int = 2):
        ...
        if api_key_path is not None:
            with open(api_key_path, 'r') as fp:
                try:
                    api_key = json.load(fp)["OPENROUTER_API_KEY"]
                except KeyError:
    ...
    def generate_srs(self, program_description: str) -> Union[str, tuple]:
        ...
        if self.__quick_check(description=program_description):
            ...
            if response is not None and len(response) > 0:
                return (response[response.find(srs_begin) + len(srs_begin) + 1
                                :response.find(srs_end)].strip('\n'), '')
    ...
    def correct_syntax(self,
        ...
        :return: This method returns the corrected NAR representation.
        """
        if self.__quick_check(nar_code=nar_code):
            ...
            if response is not None and len(response) > 0:
                return (response[response.find(nar_begin) + len(nar_begin) + 1
                                    :response.find(nar_end)].strip('\n'), '')
    ...
    def refine_contracts(self,
        ...
        :return: This method returns the extended set of contracts.
        """
        if self.__quick_check(contracts=contracts):
            ...
            if response is not None and len(response) > 0:
                return (response[response.find(contracts_begin) + len(contracts_begin) + 1
                                :response.find(contracts_end)].strip('\n'), '')
    ...
    def reflective_reasoning(self, contracts: str, verification_plan: str, batch_samples: int) -> Union[str, tuple]:
        ...
        if response is not None and len(response) > 0:
            return (response[response.find(contracts_begin) + len(contracts_begin) + 1
                            :response.find(contract_end)].strip('\n'), '', list(zip(batch, batch_exps)))

    def sphinx(self, contracts: str) -> Union[str, tuple]:
        """
        ...
        :param contracts: The set of contracts that were generated by Pyramid.
        :return: This method returns the revised set of contracts.
        """
        prompt = self.__prompts['sphinx'].replace('INPUT_CONTRACTS', contracts).replace("PROGRAM_DESCRIPTION", self.__pd)
    ...
    def generate_ToT(self,
        ...
        if self.__quick_check(description=program_description):
            ...
            evaluation_response = self.__model.chat.completions.create(
                extra_body={},
                model=self.__model_id,
                max_completion_tokens=8192,
                messages=[
    ...
    def reconstruct_program(self, corrupted_code: str, specification: str) -> str:
        ...
        if response is not None and len(response) > 0:
            return response[response.find(recon_begin) + len(recon_begin) + 1
                            :response.find(recon_end)].strip('\n')
...
if __name__ == '__main__':
    ...
    ins = NectronOpenAI(model_id='openai/gpt-oss-20b') # OpenRouter Model ID.
    ins.InitClient(api_key="")
