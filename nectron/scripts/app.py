...
from .nectron import NonCompilableNAR
from openai import AuthenticationError
from .nectron_ablation import NectronForAblation
...
class NectronApp:
    ...
    def __init__(self):
        ...
        self.__current_state = NectronState(*(['']*len(NectronState.__dict__['__static_attributes__'])))
        ...
        with self.__menu_bar:
            ...
            with self.__view_menu:
                ...
                with dpg.tooltip(NAR_MENU_ITEM, delay=0.5):
                    dpg.add_text('Alt + 2')
                self.__sc_menu_view = dpg.add_menu_item(tag=SC_MENU_ITEM, label="Contracts (From The Symbolic Engine)", check=True,
    ...
    def __settings(self):
        if dpg.get_value(SETTINGS_MENU_ITEM):
            ...
            with dpg.window(tag=SETTINGS_VIEW, label='Settings', pos=(85, 230), width=1000, height=510,
                ...
                dpg.add_radio_button(['OpenRouter'], horizontal=True,
                ...
                dpg.add_text(default_value='Number of Refinement Calls (Higher Number, Slower Inference)')
    ...
    def __contracts_seeds_log(self):
        ...
        if dpg.get_value(SC_MENU_ITEM):
            ...
            dpg.add_text('Contracts (From The Symbolic Engine)', parent=MAIN_WINDOW, tag=SC_VIEW_LABEL)
    ...
    def run(self):
        ...
        with dpg.handler_registry():
            dpg.add_key_down_handler(dpg.mvKey_LControl, callback=self.__shortcuts)
            dpg.add_key_down_handler(dpg.mvKey_LAlt, callback=self.__view_shortcuts)
            dpg.add_key_down_handler(dpg.mvKey_LShift, callback=self.__shift_shortcuts)
    ...
    def __nectron_init(self):
        ...
        if self.__configuration['api_key'] == '':
            ...
            try:
                ...
                openrouter_id = self.__default_openrouter if not current_model_id else current_model_id

                self.__nectron = NectronForAblation(api_key=self.__configuration['api_key'],
    ...
    def __generate_contracts(self, description: str):
        if self.__nectron is not None:
            ...
            if correction_amount > 0 and grammar_ok:
                ...
                suggestions.insert(0, [revised_contracts])
