...
from scripts.nectron_ablation import NectronForAblation
...
class NectronInferrer:
    ...
    def infer_nectron(self, rri: int, num_tots: int, sleep: bool = False):
        ...
        for identifier in self.selected_models:
            ...
            os.makedirs(os.path.join(self.output_dir, f'{model_name.lower()}', 
                                     'code_with_sphinx_contracts'), exist_ok=True)
            os.makedirs(os.path.join(self.output_dir, f'{model_name.lower()}', 
                                     'code_with_simple_refined_contracts'), exist_ok=True)
            os.makedirs(os.path.join(self.output_dir, f'{model_name.lower()}', 
                                     'code_with_reflective_refined_contracts'), exist_ok=True)
            os.makedirs(os.path.join(self.output_dir, f'{model_name.lower()}', 
                                     'code_with_seeds'), exist_ok=True)
            
            os.makedirs(os.path.join(self.output_dir, f'{model_name.lower()}',
                                      'reconstruction_with_sphinx_contracts'), exist_ok=True)
            os.makedirs(os.path.join(self.output_dir, f'{model_name.lower()}',
                                      'reconstruction_with_simple_refined_contracts'), exist_ok=True)
            os.makedirs(os.path.join(self.output_dir, f'{model_name.lower()}',
                                      'reconstruction_with_reflective_refined_contracts'), exist_ok=True)
            os.makedirs(os.path.join(self.output_dir, f'{model_name.lower()}', 
                                     'reconstruction_with_seeds'), exist_ok=True)
            
            model = NectronForAblation(api_key=self.openrouter_api_key, openrouter_model_id=identifier)
            ...
            for index, program in enumerate(programs[start_index:end_index+1]):
                ...
                if evaluation['ablations']:
                    fused_sphinx_code = evaluation['ablations']['augmented_refinement'] + '\n' + program['c_implementation']
                    fused_simple_refined_code = evaluation['ablations']['simple_refinement'] + '\n' + program['c_implementation']
                    fused_reflective_refined_code = evaluation['ablations']['reflective_refinement'] + '\n' + program['c_implementation']
                else:
                    fused_sphinx_code = f'//{evaluation['acsl_contracts']}\n' + program['c_implementation']
                    fused_simple_refined_code = f"//{evaluation['acsl_contracts']}\n" + '\n' + program['c_implementation']
                    fused_reflective_refined_code = f"//{evaluation['acsl_contracts']}\n" + '\n' + program['c_implementation']
                ...
                fused_sphinx_reconstructions, fused_seeds_reconstructions = [], []
                fused_reflective_refined_reconstructions = []
                fused_simple_refined_reconstructions = []
                sphinx_reconstructions, seeds_reconstructions = [], []
                reflective_refined_reconstructions = []
                simple_refined_reconstructions = []
                ...
                for idy, corrupted_version in enumerate(program['c_corrupted_implementation']):
                    ...
                    # while not flag:
                    #     try:
                    if not evaluation['ablations']:

                        reconstruction_with_sphinx_contracts = "//Couldn't reconstruct, Non-Compilable NAR\n" + corrupted_version
                        reconstruction_with_reflective_refined_contracts = "//Couldn't reconstruct, Non-Compilable NAR\n" + corrupted_version
                        reconstruction_with_simple_refined_contracts = "//Couldn't reconstruct, Non-Compilable NAR\n" + corrupted_version
                        reconstruction_with_seeds = "//Couldn't reconstruct, Non-Compilable NAR\n" + corrupted_version
                    
                    else:
                        reconstruction_with_sphinx_contracts = model.reconstruct_program(corrupted_code=masked_corrupted_code, 
                                                                specification=evaluation['ablations']['augmented_refinement']).replace('foo', program['function_name'])
                        reconstruction_with_sphinx_contracts = self.ip.preprocess(reconstruction_with_sphinx_contracts)

                        reconstruction_with_reflective_refined_contracts = model.reconstruct_program(corrupted_code=masked_corrupted_code, 
                                                                specification=evaluation['ablations']['reflective_refinement']).replace('foo', program['function_name'])
                        reconstruction_with_reflective_refined_contracts = self.ip.preprocess(reconstruction_with_reflective_refined_contracts)

                        reconstruction_with_simple_refined_contracts = model.reconstruct_program(corrupted_code=masked_corrupted_code, 
                                                                specification=evaluation['ablations']['simple_refinement']).replace('foo', program['function_name'])
                        reconstruction_with_simple_refined_contracts = self.ip.preprocess(reconstruction_with_simple_refined_contracts)
                    ...
                    if evaluation['acsl_contracts'].find('ScopeError') != -1:
                        fused_sphinx_acsl_reconstruction = self.ip.preprocess(reconstruction_with_sphinx_contracts)
                        fused_reflective_refined_acsl_reconstruction = self.ip.preprocess(reconstruction_with_reflective_refined_contracts)
                        fused_simple_refined_acsl_reconstruction = self.ip.preprocess(reconstruction_with_simple_refined_contracts)
                    else:
                        fused_sphinx_acsl_reconstruction = self.ip.preprocess(evaluation['acsl_contracts'] + '\n' + reconstruction_with_sphinx_contracts)
                        fused_reflective_refined_acsl_reconstruction = self.ip.preprocess(evaluation['ablations']['reflective_refinement'] + '\n' + reconstruction_with_reflective_refined_contracts)
                        fused_simple_refined_acsl_reconstruction = self.ip.preprocess(evaluation['ablations']['simple_refinement'] + '\n' + reconstruction_with_simple_refined_contracts)
                    ...
                    fused_sphinx_reconstructions.append(fused_sphinx_acsl_reconstruction)
                    fused_reflective_refined_reconstructions.append(fused_reflective_refined_acsl_reconstruction)
                    fused_simple_refined_reconstructions.append(fused_simple_refined_acsl_reconstruction)
                    fused_seeds_reconstructions.append(fused_seeds_acsl_reconstruction)

                    sphinx_reconstructions.append(
                        reconstruction_with_sphinx_contracts.replace('//ScopeError: The provided prompt is outside the scope of contracts that NECTRON can generate.\n', '')
                    )
                    reflective_refined_reconstructions.append(
                        reconstruction_with_reflective_refined_contracts.replace('//ScopeError: The provided prompt is outside the scope of contracts that NECTRON can generate.\n', '')
                    )
                    simple_refined_reconstructions.append(
                        reconstruction_with_simple_refined_contracts.replace('//ScopeError: The provided prompt is outside the scope of contracts that NECTRON can generate.\n', '')
                    )
                    ...
                    with open(os.path.join(self.output_dir, f'{model_name.lower()}', 
                                           'reconstruction_with_sphinx_contracts', f'program_{index+start_index}_{idy+1}.c'), 'w') as fp:
                        fp.write(fused_sphinx_acsl_reconstruction)

                    with open(os.path.join(self.output_dir, f'{model_name.lower()}', 
                                           'reconstruction_with_reflective_refined_contracts', f'program_{index+start_index}_{idy+1}.c'), 'w') as fp:
                        fp.write(fused_reflective_refined_acsl_reconstruction)

                    with open(os.path.join(self.output_dir, f'{model_name.lower()}', 
                                           'reconstruction_with_simple_refined_contracts', f'program_{index+start_index}_{idy+1}.c'), 'w') as fp:
                        fp.write(fused_simple_refined_acsl_reconstruction)
                ...
                results['inferred_programs'].append({
                    ...
                    'fused_sphinx_code': fused_sphinx_code,
                    'fused_simple_refined_acsl_code': fused_simple_refined_code,
                    'fused_reflective_refined_acsl_code': fused_reflective_refined_code,
                    'fused_seeds_acsl_code': fused_seeds_acsl_code,
                    'fused_sphinx_reconstruction': fused_sphinx_reconstructions,
                    'fused_reflective_refined_reconstruction': fused_reflective_refined_reconstructions,
                    'fused_simple_refined_reconstruction': fused_simple_refined_reconstructions,
                    'fused_seeds_acsl_reconstruction': fused_seeds_reconstructions,
                    'adl_compiled': compiled,
                    'reconstruction_with_sphinx_contracts': sphinx_reconstructions,
                    'reconstruction_with_reflective_refined_contracts': reflective_refined_reconstructions,
                    'reconstruction_with_simple_refined_contracts': simple_refined_reconstructions,
                ...
                with open(os.path.join(self.output_dir, f'{model_name.lower()}', 
                                       'code_with_sphinx_contracts', f'program_{index+start_index}.c'), 'w') as fp:
                    fp.write(fused_sphinx_code)

                with open(os.path.join(self.output_dir, f'{model_name.lower()}', 
                                       'code_with_reflective_refined_contracts', f'program_{index+start_index}.c'), 'w') as fp:
                    fp.write(fused_reflective_refined_code)

                with open(os.path.join(self.output_dir, f'{model_name.lower()}', 
                                       'code_with_simple_refined_contracts', f'program_{index+start_index}.c'), 'w') as fp:
                    fp.write(fused_simple_refined_code)
    ...
    def infer_zero_shot(self, sleep: bool = False):
        ...
        for identifier in self.selected_models:
            ...
            model = NectronForAblation(api_key=self.openrouter_api_key, 
                            openrouter_model_id=identifier)
    ...
    def evaluate(self, model_exclusion: list = [], zero_shot: bool=False):
        ...
        for modelf in os.listdir(self.inference_dir):
            ...
            if zero_shot:
                ...
                model_results = {'sphinx_acsl': [], 'reflective_refinement_acsl': [],
                                 'simple_refinement_acsl': [], 'seeds_acsl': [],
                            'sphinx_reconstructions': [],
                            'simple_refinement_reconstructions': [],
                            'reflective_refinement_reconstructions': [],
            ...
            for subfolder in os.listdir(os.path.join(self.inference_dir, modelf)):
                if not subfolder.endswith('.json'):
                    ...
                    save_key = ''
                    if subfolder == 'code_with_sphinx_contracts':
                        save_key = 'sphinx_acsl'
                    elif subfolder == 'code_with_reflective_refined_contracts':
                        save_key = 'reflective_refinement_acsl'
                    elif subfolder == 'code_with_simple_refined_contracts':
                        save_key = 'simple_refinement_acsl'
                    elif subfolder == 'code_with_seeds':
                        save_key = 'seeds_acsl'
                    elif subfolder == 'reconstruction_with_sphinx_contracts':
                        save_key = 'sphinx_reconstructions'
                    elif subfolder == 'reconstruction_with_reflective_refined_contracts':
                        save_key = 'reflective_refinement_reconstructions'
                    elif subfolder == 'reconstruction_with_simple_refined_contracts':
                        save_key = 'simple_refinement_reconstructions'
            ...
            if not zero_shot:
                pairs = [('sphinx_acsl', 'sphinx_reconstructions'), 
                         ('reflective_refinement_acsl', 'reflective_refinement_reconstructions'),
                         ('simple_refinement_acsl', 'simple_refinement_reconstructions'),
                         ('seeds_acsl', 'seeds_reconstructions')]
                
                sphinx_results = []
                simple_refinement_results = []
                reflective_refinement_results = []
                seeds_results = []

                performance = {'sphinx': 0, 'reflective_refinement': 0,
                                'simple_refinement': 0, 'seeds': 0}
            ...
            for pair in pairs:
                for program in model_results[pair[0]]:
                    ...
                    elif pair[0] == 'sphinx_acsl':
                        performance['sphinx'] += adherence_score / len(model_results[pair[0]])
                        sphinx_results.append({
                            "alpha": adherence_score,
                            "program_score": program_score,
                            "reconstruction_mean_score": r_mean_score,
                            "reconstructions_scores": reconstruction_scores
                        })
                    elif pair[0] == 'reflective_refinement_acsl':
                        performance['reflective_refinement'] += adherence_score / len(model_results[pair[0]])
                        reflective_refinement_results.append({
                            "alpha": adherence_score,
                            "program_score": program_score,
                            "reconstruction_mean_score": r_mean_score,
                            "reconstructions_scores": reconstruction_scores
                        })
                    elif pair[0] == 'simple_refinement_acsl':
                        performance['simple_refinement'] += adherence_score / len(model_results[pair[0]])
                        simple_refinement_results.append({
                            "alpha": adherence_score,
                            "program_score": program_score,
                            "reconstruction_mean_score": r_mean_score,
                            "reconstructions_scores": reconstruction_scores
            ...
            if not zero_shot:
                print(f"\nModel: {modelf} has achieved a performance of {performance['sphinx'] * 100:.3f}% (Sphinx),"
                      f"  {performance['reflective_refinement'] * 100:.3f}% (Reflective Refinement),"
                      f"  {performance['simple_refinement'] * 100:.3f}% (Simple Refinement),"
            ...
            if not zero_shot:
                with open(os.path.join(self.save_dir, modelf, 'sphinx_eval.json'), 'w') as fp:
                    json.dump(sphinx_results, fp, indent=4)
                with open(os.path.join(self.save_dir, modelf, 'reflective_refinement_eval.json'), 'w') as fp:
                    json.dump(reflective_refinement_results, fp, indent=4)
                with open(os.path.join(self.save_dir, modelf, 'simple_refinement_eval.json'), 'w') as fp:
                    json.dump(simple_refinement_results, fp, indent=4)
                with open(os.path.join(self.save_dir, modelf, 'seeds_eval.json'), 'w') as fp:
                    json.dump(seeds_results, fp, indent=4)
            else:
                with open(os.path.join(self.save_dir, modelf, 'zero_shot_eval.json'), 'w') as fp:
                    json.dump(zero_shot_results, fp, indent=4)
...
if __name__ == '__main__':
    ...
    # neval = NectronEvaluator(inference_dir='Ablation/nectron_inference',
    #                          save_dir='Ablation/nectron_ablation_eval',
    #                          rri=2, tot=1) # 2


    # neval.evaluate(zero_shot=False, model_exclusion=['gpt-oss-20b']) # 2
