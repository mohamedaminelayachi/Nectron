...
class NectronOpenAI:
    ...
    def __init__(self, model_id: str, explanation: bool = True):
        self.__api_key = None
        self.__client = None
        self.__model = None
            ...
            if self.__explanation:
                if response is not None and len(response) > 0:
                    return (response[response.find(srs_begin) + len(srs_begin) + 1
                            ...
                            response[response.find(exp_begin) + len(exp_begin) + 1:
                            response.find(exp_end)].strip('\n'))
            else:
                if response is not None and len(response) > 0:
                    return (response[response.find(srs_begin) + len(srs_begin) + 1
                                    :response.find(srs_end)].strip('\n'), '')
        ...
        if self.__explanation:
            if response is not None and len(response) > 0:
                return (response[response.find(contracts_begin) + len(contracts_begin) + 1
                                    :response.find(contract_end)].strip('\n'),
                        response[
                        response.find(exp_begin) + len(exp_begin) + 1:
                        response.find(exp_end)].strip('\n'))
        else:
            if response is not None and len(response) > 0:
                return (response[response.find(contracts_begin) + len(contracts_begin) + 1
...
if __name__ == '__main__':
    ...
    ins.InitClient(api_key="YOUR_OPENROUTER_API_KEY")
